...
client = OpenAI(
  api_key='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'
)
...
for i in range(len(df)):
    ...
    chat_completion = client.chat.completions.create(
        messages=[
            {
                "role": "user",
                "content": "Suppose you are playing Balderdash and you want to deceive your opponants with a deceiving definition. define "+ df['words'][i] +" in one sentence.",
            }
        ],
        model="gpt-3.5-turbo",
    )
