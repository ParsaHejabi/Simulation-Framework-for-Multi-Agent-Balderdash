...
class evaluation_tools:
    ...
    def get_round_data(self, game_id: int, round_id: int):
        ...
        if round:
            ...
            for llm in list(player_types_inverse.keys()):
                ...
                deceptions = [
                    deception[player] for player in player_types_inverse[llm] if deception[player] != -1
                ]
                round_data["deception_ratio"][llm] = np.mean(deceptions) if len(deceptions) > 0 else -1
                round_data["correct_guesse_ratio"][llm] = (
                    correct_guesse[llm] / (len(player_types_inverse[llm]) - true_def[llm])
                    if len(player_types_inverse[llm]) - true_def[llm] != 0
                    else -1
                )
            ...
            if game_data != None:
                experiment_data.setdefault(
                    (
                        ...
                        game["correct_definition_points"],
                    ),
                    {},
                )[i] = game_data
            else:
    ...
    def experiment_game_average(self, experiment_data: dict):
        ...
        for llm in llm_players:
            average_data[llm] = {
                "llm_knows_ratio": {
                    key: np.mean([ratio[llm] for ratio in value["llm_knows_ratio"]])
                    for key, value in experiment_data.items()
                },
                "true_def_ratio": {
                    key: np.mean([ratio[llm] for ratio in value["true_def_ratio"]])
                    for key, value in experiment_data.items()
                },
                "deception_ratio": {
                    key: np.mean([i for i in [ratio[llm] for ratio in value["deception_ratio"]] if i != -1])
                    for key, value in experiment_data.items()
                },
                "correct_guesse_ratio": {
                    key: np.mean(
                        [i for i in [ratio[llm] for ratio in value["correct_guesse_ratio"]] if i != -1]
                    )
                    for key, value in experiment_data.items()
                },
                "scores_ratio": {
                    key: np.mean([ratio[llm] for ratio in value["scores_ratio"]])
                    for key, value in experiment_data.items()
                },
            }

            overal_averge_data[llm] = {
                "llm_knows_ratio": {
                    "mean": np.mean(list(average_data[llm]["llm_knows_ratio"].values())),
                    "std": np.std(list(average_data[llm]["llm_knows_ratio"].values())),
                },
                "true_def_ratio": {
                    "mean": np.mean(list(average_data[llm]["true_def_ratio"].values())),
                    "std": np.std(list(average_data[llm]["true_def_ratio"].values())),
                },
                "deception_ratio": {
                    "mean": np.mean(list(average_data[llm]["deception_ratio"].values())),
                    "std": np.std(list(average_data[llm]["deception_ratio"].values())),
                },
                "correct_guesse_ratio": {
                    "mean": np.mean(list(average_data[llm]["correct_guesse_ratio"].values())),
                    "std": np.std(list(average_data[llm]["correct_guesse_ratio"].values())),
                },
                "scores_ratio": {
                    "mean": np.mean(list(average_data[llm]["scores_ratio"].values())),
    ...
    def experiment_round_average(self, experiment_data: dict):
        ...
        return {
            "llm_knows_ratio": {"mean": avg_llm_knows, "std": std_llm_knows},
    ...
    def plot_true_def_ratio(self, experiment_data: dict, word_filter: str, word_file: str, window_size: int):
        ...
        for i, llm in enumerate(llm_players):
            avg_true_def_smooth = (
                np.convolve(round_average["true_def_ratio"]["mean"][llm], np.ones(window_size), "valid")
                / window_size
            )
            avg_llm_knows_smooth = (
                np.convolve(round_average["llm_knows_ratio"]["mean"][llm], np.ones(window_size), "valid")
                / window_size
            )
            std_true_def_smooth = (
                np.convolve(round_average["true_def_ratio"]["std"][llm], np.ones(window_size), "valid")
                / window_size
            )
            std_llm_knows_smooth = (
                np.convolve(round_average["llm_knows_ratio"]["std"][llm], np.ones(window_size), "valid")
                / window_size
            )
            ...
            ax.fill_between(
                range(n + 1 - window_size),
                avg_true_def_smooth - std_true_def_smooth,
                avg_true_def_smooth + std_true_def_smooth,
                alpha=0.2,
            )
            ...
            ax.fill_between(
                range(n + 1 - window_size),
                avg_llm_knows_smooth - std_llm_knows_smooth,
                avg_llm_knows_smooth + std_llm_knows_smooth,
                alpha=0.2,
            )
    ...
    def plot_scores_ratio(self, experiment_data: dict, word_filter: str, word_file: str, window_size: int):
        ...
        for llm in llm_players:
            avg_scores_smooth = (
                np.convolve(round_average["scores_ratio"]["mean"][llm], np.ones(window_size), "valid")
                / window_size
            )
            std_scores_smooth = (
                np.convolve(round_average["scores_ratio"]["std"][llm], np.ones(window_size), "valid")
                / window_size
            )
            ...
            plt.fill_between(
                range(n + 1 - window_size),
                avg_scores_smooth - std_scores_smooth,
                avg_scores_smooth + std_scores_smooth,
                alpha=0.2,
            )
        plt.xlabel("Rounds")
