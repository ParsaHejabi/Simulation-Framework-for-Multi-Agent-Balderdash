...
def main() -> None:
    ...
    parser.add_argument("--experiment_type", type=str, help="Type of experiment can be 'benchmark' or 'regression'")
    parser.add_argument("--output_dir", type=str, help="Output directory to save the results", default=".")
    
    args = parser.parse_args()

    etools = evaluation_tools(os.getenv("MONGODB_CONNECTION_STRING"),
                          'balderdash_game')
    ...
    if args.experiment_type == 'benchmark':
        ...
        for llm, data in regression_stats.items():
            ...
            regression_results[llm]['slope'], regression_results[llm]['intercept'], regression_results[llm]['r_value'], regression_results[llm]['p_value'], regression_results[llm]['std_err'] = stats.linregress(list(data.keys()), list(data.values()))
        df = pd.DataFrame(regression_results).T
        df.index.name = 'LLM'
        print(df)
        df.to_csv(os.path.join(args.output_dir, 
                               f"regression_experiment_{args.start_game_id}_{args.end_game_id}.csv"))
