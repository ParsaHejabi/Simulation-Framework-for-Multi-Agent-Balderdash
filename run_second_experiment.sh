...
for player_llm_models in $player_llm_models_list; do
    ...
    for history_type in $history_types_list; do
        ...
        # if $history_type is "full", history_prompt_file is "prompts/full_history.txt"
        # if $history_type is "mini", history_prompt_file is "prompts/mini_history.txt"
        ...
        if [ $dry_run = true ]; then
            ...
            game_description="Convergence experiment for player_llm_models: ${player_llm_models}, history_type: ${history_type} with default scoring rules (${receiving_vote_points}, ${correct_vote_points}, ${correct_definition_points}) and ${#random_seeds[@]} different seeds, no communication, no seed stories."
        fi

        # Calculate total number of runs upfront
        total_runs=$((${#random_seeds[@]} * (${#player_llm_models_list[@]} * ${#history_types_list[@]})))

        current_run=0

        for random_seed in "${random_seeds[@]}"; do
            current_run=$((current_run + 1))
            echo "Running ${current_run}/${total_runs} with player LLM model ${player_llm_models}, history type ${history_type}, and random seed ${random_seed}"
